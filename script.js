...
function copyLastRecord() {
    ...
    if (lastRecord.receiptImages && lastRecord.receiptImages.length > 0) {
        ...
        updateImagePreview();
    } else if (lastRecord.receiptImage) {
        // 向後相容：單張圖片
        window.selectedReceiptImages = [lastRecord.receiptImage];
        updateImagePreview();
    }
...
function testTodayRecords() {
    ...
    console.log('今天的記錄數:', todayRecords.length);
...
function updateAllPageDisplays() {
    console.log('更新所有頁面顯示...');
    
    // 總是更新記帳本頁面的數據（即使當前不可見）
    if (typeof initLedger === 'function') {
        console.log('強制更新記帳本頁面數據...');
        initLedger();
    }
    ...
    if (pageAccounts && pageAccounts.style.display !== 'none') {
    ...
    if (pageInvestment && pageInvestment.style.display !== 'none') {
        if (typeof initInvestmentPage === 'function') {
            initInvestmentPage();
        }
    }
...
function initLedger() {
    ...
    console.log('初始化記帳本頁面，總記錄數:', records.length);
    ...
    console.log('類型篩選後記錄數:', filteredRecords.length);
    
    // 應用搜尋和篩選條件
    filteredRecords = applyAllFilters(filteredRecords);
    console.log('所有篩選後記錄數:', filteredRecords.length);
    ...
    console.log('是否有日期篩選:', hasDateFilter);
...
function displayLedgerTransactions(records, showAll = false) {
    ...
    console.log('顯示交易記錄，記錄數:', records.length, 'showAll:', showAll);
    ...
    console.log('分組後的日期:', Object.keys(grouped));
    ...
    console.log('排序後的日期:', sortedDates);
    ...
    displayDates.forEach(dateKey => {
        ...
        grouped[dateKey].forEach((record, index) => {
            ...
            const images = record.receiptImages || (record.receiptImage ? [record.receiptImage] : []);
            if (images.length > 0) {
                receiptImageHtml = `
                    <div class="transaction-receipt" style="margin-top: 8px;">
                        <div class="receipt-images-container" style="display: flex; gap: 4px; flex-wrap: wrap;">
                            ${images.map((image, index) => `
                                <img src="${image}" alt="收據${index + 1}" class="receipt-thumbnail" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px; cursor: pointer; border: 2px solid #e0e0e0;" data-receipt-image="${image}">
                            `).join('')}
                        </div>
                    </div>
                `;
                ...
                if (images.length > 0) {
                    receiptImageHtml = `
                        <div class="history-receipt-container">
                            <div class="history-receipt-images-container" style="display: flex; gap: 4px; flex-wrap: wrap;">
                                ${images.map((image, index) => `
                                    <img src="${image}" alt="收據${index + 1}" class="history-receipt-thumbnail" data-receipt-image="${image}">
                                `).join('')}
                            </div>
                        </div>
                    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    imageInput.multiple = true; // 支援多選
    document.body.appendChild(imageInput);
    
    // 初始化圖片陣列
    window.selectedReceiptImages = window.selectedReceiptImages || [];
    ...
    imageInput.addEventListener('change', async (e) => {
        ...
        // 檢查總圖片數量限制
        const currentCount = window.selectedReceiptImages.length;
        const maxImages = 5;
        const minImages = 3;
        
        if (currentCount + files.length > maxImages) {
            alert(`最多只能上傳 ${maxImages} 張圖片！目前已有 ${currentCount} 張。`);
            imageInput.value = '';
            return;
        }
...
function updateImagePreview() {
    ...
    container.innerHTML = images.map((image, index) => `
        <div class="image-preview-item" style="position: relative; display: inline-block; margin: 4px;">
            <img src="${image}" alt="圖片${index + 1}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; border: 2px solid #e0e0e0;">
            <button class="image-remove-single-btn" data-index="${index}" style="position: absolute; top: -8px; right: -8px; background: #ff4444; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center;">✕</button>
        </div>
    `).join('');
    
    // 更新計數
    if (imageCount) imageCount.textContent = `${images.length}/5`;
    
    // 綁定移除按鈕事件
    container.querySelectorAll('.image-remove-single-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const index = parseInt(btn.dataset.index);
    ...
    // 控制添加按鈕顯示
    if (addImageBtn) addImageBtn.style.display = images.length >= 5 ? 'none' : 'inline-block';
}
