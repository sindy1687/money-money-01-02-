...
function showHistoryBackgroundSelector(modalContent) {
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        if (file) {
            console.log(`處理背景圖片: ${file.name}，大小: ${Math.round(file.size/1024/1024)}MB`);
...
function generateColors(count, palette) {
    ...
    if (customTheme.chartColors && customTheme.chartColors.length > 0) {
        ...
        // 檢查是否有有效的顏色值
        const hasValidColors = primaryColor && primaryLight && primaryLighter && primaryDark &&
            primaryColor.startsWith('#') && primaryLight.startsWith('#') && 
            primaryLighter.startsWith('#') && primaryDark.startsWith('#') ? 
                {
                    primary: hexToRgb(primaryColor),
                    light: hexToRgb(primaryLight),
                    lighter: hexToRgb(primaryLighter),
                    dark: hexToRgb(primaryDark)
                } : null;
        
        // 生成主題相關的顏色系列
        if (hasValidColors.primary && hasValidColors.light && hasValidColors.lighter && hasValidColors.dark) {
            // 根據主題顏色生成漸變色系列
            baseColors = [
                { bg: `rgba(${hasValidColors.primary.r}, ${hasValidColors.primary.g}, ${hasValidColors.primary.b}, 0.8)`, border: `rgba(${hasValidColors.primary.r}, ${hasValidColors.primary.g}, ${hasValidColors.primary.b}, 1)` },
                { bg: `rgba(${hasValidColors.light.r}, ${hasValidColors.light.g}, ${hasValidColors.light.b}, 0.8)`, border: `rgba(${hasValidColors.light.r}, ${hasValidColors.light.g}, ${hasValidColors.light.b}, 1)` },
                { bg: `rgba(${hasValidColors.lighter.r}, ${hasValidColors.lighter.g}, ${hasValidColors.lighter.b}, 0.8)`, border: `rgba(${hasValidColors.lighter.r}, ${hasValidColors.lighter.g}, ${hasValidColors.lighter.b}, 1)` },
                { bg: `rgba(${hasValidColors.dark.r}, ${hasValidColors.dark.g}, ${hasValidColors.dark.b}, 0.8)`, border: `rgba(${hasValidColors.dark.r}, ${hasValidColors.dark.g}, ${hasValidColors.dark.b}, 1)` }
            ];
            
            // 如果需要更多顏色，生成漸變
            while (baseColors.length < count) {
                const lastColor = baseColors[baseColors.length - 1];
                const newOpacity = Math.max(0.3, parseFloat(lastColor.bg.match(/[\d.]+\)$/)[0]) - 0.1);
                const bgMatch = lastColor.bg.match(/rgba\((\d+), (\d+), (\d+),/);
                if (bgMatch) {
                    baseColors.push({
                        bg: `rgba(${bgMatch[1]}, ${bgMatch[2]}, ${bgMatch[3]}, ${newOpacity})`,
                        border: lastColor.border
                    });
                } else {
                    break;
                }
            ...
            baseColors = [
                ...
                { bg: 'rgba(74, 144, 226, 0.8)', border: 'rgba(74, 144, 226, 1)' },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    imageInput.addEventListener('change', async (e) => {
        ...
        if (files.length > 0) {
            ...
            console.log(`準備上傳 ${files.length} 張圖片，現有 ${window.selectedReceiptImages.length} 張，總共 ${totalImages} 張`);
            
            // 移除預估總大小檢查
            console.log('跳過大小檢查，允許所有圖片上傳');
            
            // 處理每個檔案
            for (const file of files) {
                // 移除單張圖片大小限制
                console.log(`處理圖片: ${file.name}，大小: ${Math.round(file.size/1024/1024)}MB`);
...
function uploadImageEmoji(container) {
    ...
    input.addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (file) {
            console.log(`處理表情圖片: ${file.name}，大小: ${Math.round(file.size/1024/1024)}MB`);
            const reader = new FileReader();
            reader.onload = (event) => {
                // 直接使用原始圖片，不進行裁切
                const imageData = event.target.result;
                console.log('使用原始表情圖片，不進行裁切');
                
                // 創建表情元素
                const emojiElement = document.createElement('div');
                emojiElement.className = 'emoji-item image-emoji';
                emojiElement.innerHTML = `
                    <img src="${imageData}" alt="自定義表情" style="width: 40px; height: 40px; object-fit: cover; border-radius: 8px;">
                    <button class="emoji-remove" style="position: absolute; top: -4px; right: -4px; background: #ff4444; color: white; border: none; border-radius: 50%; width: 16px; height: 16px; font-size: 10px; cursor: pointer;">×</button>
                `;
                
                // 添加點擊事件
                emojiElement.addEventListener('click', () => {
                    const emojiInput = container.closest('.emoji-input-container').querySelector('.emoji-input');
                    if (emojiInput) {
                        emojiInput.value = imageData;
                        emojiInput.dataset.type = 'image';
                    }
                });
                
                // 添加移除事件
                emojiElement.querySelector('.emoji-remove').addEventListener('click', (e) => {
                    e.stopPropagation();
                    emojiElement.remove();
                });
...
function initAccountImageUpload() {
    ...
    imageInput.addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (file) {
            console.log(`處理帳戶圖片: ${file.name}，大小: ${Math.round(file.size/1024/1024)}MB`);
            const reader = new FileReader();
            reader.onload = (event) => {
                // 直接使用原始圖片，不進行裁切
                const imageData = event.target.result;
                console.log('使用原始帳戶圖片，不進行裁切');
